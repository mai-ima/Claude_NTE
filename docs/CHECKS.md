...
### 4-5. 公開ページの禁止表現（ソースを読む）

`src/pages` `src/components` `src/layouts` `src/data` `src/lib` の
`.astro` / `.ts` / `.tsx` から**コメントを取り除いてから**、画面に出る文字列に
「運営の過程」を書いた言い回しが残っていないかを見ます。

止めている語（理由つきで指摘に出る）:
`近日公開` / `決まっていません` / `せずに済み` / `と思っていましたが` /
`あえて` / `作業用ブランチ` / `main への取り込み`

**落ちたときの直し方**: 読者にとっての**結果と次の案内**だけに書き換えます。
判定表は `docs/RECIPES.md` の「6. 公開ページの文章」、
決定の原文は `.claude/state/DECISIONS.md`（2026-09-08）にあります。

> 悪い例をコメントに引用するのは通ります（コメントは検査していません）。
> どうしても本文に必要なときは `scripts/check-ui.mjs` の `BANNED_PHRASES` を見直します。

### 4-6. CSS コメントの入れ子

`src/styles/*.css` のコメントの中に `/*` が入っていないかを見ます。
CSS のコメントは入れ子にできないため、引用の中に閉じ記号があると**そこで閉じてしまい、
後続のルールが丸ごと無効**になります（実際に起きました → `docs/FINDINGS.md`）。
...
## 10) 絵文字を使っていないか（`check-ui.mjs`）

**利用者の決定**（`.claude/state/DECISIONS.md` 2026-09-14「絵文字を使わない」）。

記事本文・画面の文言・`docs/` の内部メモまで、**絵文字は使いません**。
目印が要るところは**文字**で書くか、すでにある lucide のアイコンを使います。

```
src/content/xxx/yyy.md:12: 絵文字「（ここに出る）」— 使わない決まりです（文字か lucide のアイコンに）
```

**直し方**: その絵文字を消します。意味が要るなら文字にします。

| 前 | 後 |
| --- | --- |
| 工具の絵文字（U+1F6E0）＋ `**要確認**: …` | `**要確認**: …` |
| 警告の絵文字（U+26A0）＋ `**ネタバレ注意**` | `**ネタバレ注意**` |
| 黄色い丸（U+1F7E1）＋ `黄色の…` | `黄色の…` |

> このファイル自身が検査の対象なので、**悪い例をそのまま書けません**。
> どの絵文字だったかはコードポイントで示しています。

**見逃すもの**

- `★` `☆` … レア度の表記。ゲーム内が `★6 / ★5 / ★4` と書くのでそのまま使う
- 矢印（`→` `←` `↔`）・チェック（`✓`）・かけ（`✕`）… 記号なので対象外
- `scripts/data/` … 取ってきた元データ。こちらでは直せない
