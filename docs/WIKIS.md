...
| └ うち **NTE（公式サイト風）** | 明暗どちらも | **暗いテーマ＝公式のニュース一覧**（濃灰 `#1d1d1d`／3pxの区切り／ホバーで反転）、**明るいテーマ＝公式の記事ページ**（`#313131` の本文）。見出しは赤とシアンの色ズレ、目次の印は四角→ピンクの菱形 | 下部タブバー |
| エンドフィールド | 明色 | **白地**＋UIパーツの黒 `#191919`＋蛍光イエロー `#fffa00`（**実画面で実測**）。厚い帯で区切る／番号付き見出し／**−45度のハザード帯**（3px・周期6px）／下層は**上白・下黄の2段ベタの帯**＋黒い正方形に黄のアイコン | **右下ボタン → 全画面シート** |
| 原神 | 明色 | クリームの地＋**白い6pxの太枠**（実測7回）。ヘッダーは**暗い半透明の帯**、CTA は**黄色 `#ffcf0d`**（画素から採取）、角丸 4px | （1ページのみ） |
| 鳴潮 | **暗色** | 黒 `#000`＋**金** `#dab67d`（実測）。**極細1pxの罫線**／角丸は**左上と右下だけ** | （1ページのみ） |
| スターレイル | 明色 | 白地＋太い黒＋黄の面。**右上だけ角丸**（実測11回）／黒帯 `#121212` の英字メニュー・現在地は**青 `#307af7`**・CTAは**金 `#c9af85`**（すべて画素から採取） | （1ページのみ） |
