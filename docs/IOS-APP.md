...
> ## ⚠ いまの位置づけ（2026-09-13 更新）
>
> **アプリを作る予定はありません。**
> 利用者の指示（同日・後から出たもの）:
>
> > IOSのネイティブアプリ化は今のところは予定はありませんが
> > 将来的には可能性は0ではないのでそれ用の文章も追加してください
>
> この文書は**将来その気になったときに読み返すための設計メモ**として残します。
> **いま作業する対象ではありません。**
>
> ただし、ここで整えた土台（記事データの JSON 書き出し `/api/*.json`、
> 保存層 `src/lib/user-data.ts` の `ItemRef`）は、**アプリを作らなくても役に立つ**ので
> そのまま使い続けます。データベースへ移すときにもそのまま使えます。
>
> 法的文書の側も、アプリを出す場合に備えた条項を用意してあります
> （利用規約 第12条／プライバシーポリシー 第10条／特定商取引法に基づく表記 第4条）。
> **どれも「現在は提供していない」と明記したうえで**、将来の取り扱いを定めたものです。
>
> 以前の決定（「iOS アプリを新しく作る」）の原文も
> [.claude/state/DECISIONS.md](../.claude/state/DECISIONS.md) に残してあります。
