---
title: "用語集（日本版↔英語）"
category: "スタート"
order: 90
description: "属性・通貨・地名・組織などの日本版公式表記と英語表記の対応表。"
status: "verified"
updated: 2026-06-08
checked: 2026-09-06
tags: ["用語集", "対応表"]
sources:
  - label: "Game8（日本版）— 属性"
    url: "https://game8.jp/nte/783376"
  - label: "カノウィキ — 用語集"
    url: "https://wiki.kanotype.net/ja/NTE/%E7%94%A8%E8%AA%9E%E9%9B%86"
---

海外版（英語）からの直訳ではなく、**日本版で確認できた公式表記**を優先しています。未確認のものは英語表記のまま「要確認」としています。

> このページは早見の**対応表**です。各語の**意味・なぜ特殊かの解説**は [用語集（専門ページ）](/terms/) に1語ずつまとめています。

## 属性（異能連環）

| 日本版 | 英語 |
| --- | --- |
| 光 | Cosmos |
| 霊 | Anima |
| 呪 | Incantation |
| 闇 | Chaos |
| 魂 | Psyche |
| 相 | Lakshana |

リング順: 光→霊→呪→闇→魂→相→（光へ）。詳しくは[異能連環](/systems/esper-cycle/)。

## 通貨・アイテム

| 日本版 | 英語 |
| --- | --- |
| 円石（えんせき） | Annulith |
| リフトクリスタル | Riftcrystal |
| 本質サイコロ（限定） | Fabricated Dice |
| 虚質サイコロ（標準） | Solid Dice |
| トリプルキー | Tri-Key |
| ファンス | Fons |
| 甲虫コイン | Beetle Coins |
| ハンターガイド | Hunter Guides |

## 地名

| 日本版 | 英語 |
| --- | --- |
| ヘテロシティ | Hethereau / Heterocity |
| 未聞浦 | Unheard Shores |
| 橋間地 | Bridge Crossings |
| ミゲル区 | Miguel District |
| ニューホランド | New Herland District |
| ヒナタ島 | Sunward Island |
| 異象界 | Anomaly Zone |

## 組織・用語

| 日本版 | 英語 |
| --- | --- |
| 異象（アノマリー） | Anomaly |
| 異象管理局 | Bureau of Anomaly Control (BAC) |
| エイボン骨董品店 | Eibon Antique Shop |
| 鑑定士 | Appraiser（プレイヤー） |
| エスパー（異能者） | Esper |
| 異象ハンター | Anomaly Hunter |
| ハンターレベル | Hunter Level |
| 本性ピクセル | Character Pixels（戦闘スタミナ） |
| スカボロー市場 | Scarborough Fair（ガチャ） |
| 充蓄 / 失諧 | Charge / Discord（トリオ反応） |
| 軌道外領域 | Beyond the Rails（高難度エンド） |
| ザ・サークル | The Circle（組織） |
| サークル懸賞 | Circle Bounty（バトルパス） |

> 日本版の主要用語はおおむね確定しました。版で変動する数値（凸素材量・排出率等）は引き続きゲーム内での確認を推奨します。
