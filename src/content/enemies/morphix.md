---
title: "モルフィクス（Morphix）"
type: "boss"
weakness: "Cosmos"
location: "異象狩り（「月光質屋」クリア後）"
order: 70
description: "巡礼/異象狩りのボス。弱点は光(Cosmos)。"
status: "verified"
updated: 2026-06-13
tags: ["ボス", "boss"]
sources:
  - label: "ゲームウィズ — ボス攻略まとめ"
    url: "https://gamewith.jp/nte/555902"
  - label: "神ゲー攻略 — ボス攻略と解放条件"
    url: "https://kamigame.jp/nte/page/422006965359612205.html"
---

## 概要

**モルフィクス（Morphix）** は[異象狩り](/terms/anomaly-hunter/)で周回できるボス。

## 出現場所

- 異象依頼[「月光質屋」](/terms/moonlight-pawnshop/)をクリアすると解放され、以降は周回対象になる。

## 弱点属性

- **弱点は [光（Cosmos）](/terms/el-cosmos/)**。

## 行動・ギミック

- 詳細な行動パターン・ギミックは**要確認**。

## ドロップ・報酬

- 周回で育成素材が得られる。報酬テーブルの詳細は**要確認**。

## 攻略のコツ

- [光（Cosmos）](/terms/el-cosmos/)で[ブレイク](/terms/break/)を奪い、停止中に火力を集中する。
