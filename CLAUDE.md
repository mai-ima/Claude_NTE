...
記事本文・画面の文言・コミットメッセージ・`docs/` の内部メモまで、**絵文字を使わない**。
目印が要るところは**文字**で書く（「要確認」「注意」）か、すでにある lucide のアイコンを使う。
`pnpm test:ui`（`scripts/check-ui.mjs`）が検査する。

例外は `★` `☆`（レア度の表記。ゲーム内が `★6` と書くため）と、
矢印・チェックなどの**記号**、`scripts/data/` の取得済みデータ。
