...
- **ページの重さ**: 1ページ約 83KB（サイドバーの無駄を削って 130KB から 36% 減）。
  CSS は 137KB（UIモード10種ぶん・初回のみ）
- **UIモード**: 10種。うち **`new`（次期ベース候補）だけは画面の骨組みを変える**
  （パソコン＝左の縦レール／スマホ＝浮かぶドック）。他の8種は質感を塗り替えるレイヤー。
...
準備中3つのUIは「限られた根拠に基づく解釈」なので実物を確認できたら見直す。
